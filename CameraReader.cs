    ...
    private void ReadLoop()
    {
        try
        {
            ...
            while (running)
            {
                ...
                // Debug: verify raw buffer và pixel đầu tiên
                if (frameCount == 1)
                {
                    // Dump first 50 bytes của buffer
                    Console.Write("[CameraReader] Raw buffer first 50 bytes: ");
                    for (int i = 0; i < 50; i++)
                    {
                    ...
                    Console.WriteLine($"[CameraReader] Buffer at pixel (10,0): [{idx10}]={buffer[idx10]:X2}, [{idx10+1}]={buffer[idx10+1]:X2}, [{idx10+2}]={buffer[idx10+2]:X2}, [{idx10+3}]={buffer[idx10+3]:X2}");
                }
